-- Полное исправление всех проблем с БД
-- Исправляет несоответствия схемы и функций

-- Удаляем проблемные триггеры и функции
DROP TRIGGER IF EXISTS trigger_record_tutor_earning ON lessons;
DROP TRIGGER IF EXISTS trigger_record_manager_commission ON transactions;
...
DROP FUNCTION IF EXISTS record_tutor_earning() CASCADE;
DROP FUNCTION IF EXISTS record_manager_commission() CASCADE;
DROP FUNCTION IF EXISTS calculate_tutor_earning() CASCADE;
DROP FUNCTION IF EXISTS add_manager_sale() CASCADE;
DROP FUNCTION IF EXISTS update_recurring_schedule_and_future_lessons(UUID, TIME, INTEGER) CASCADE;
...
-- Обновляем таблицу tutor_earnings
...
-- Создаем таблицу истории транзакций если не существует
...
-- Функция для начисления заработка репетитору при проведении урока
...
BEGIN
  ...
  IF NEW.status = 'completed' AND (OLD IS NULL OR OLD.status != 'completed') THEN
    ...
    IF v_tutor_id IS NOT NULL AND v_rate > 0 THEN
      ...
      -- Добавляем запись в tutor_earnings
      INSERT INTO tutor_earnings (tutor_id, lesson_id, amount, week_start, status)
      VALUES (v_tutor_id, NEW.id, v_rate, v_week_start, 'pending');
...
BEGIN
  ...
  -- Обновляем статус в tutor_earnings
...
CREATE OR REPLACE FUNCTION update_recurring_schedule_with_lessons(
  p_schedule_id UUID,
  p_new_time TIME,
...
BEGIN
  ...
  v_duration := COALESCE(p_new_duration, v_schedule.duration_minutes, 60);
  
  -- Удаляем все будущие запланированные уроки этого ученика
  DELETE FROM lessons
  ...
  -- Вычисляем следующую дату для создания уроков
  v_next_date := DATE_TRUNC('week', NOW()) + (p_new_day_of_week || ' days')::INTERVAL + p_new_time;
  ...
  INSERT INTO lessons (student_id, scheduled_at, status, duration_minutes, tutor_id, lesson_type, title)
  SELECT 
...
COMMENT ON FUNCTION update_recurring_schedule_with_lessons IS 'Обновляет регулярное расписание и пересоздает будущие уроки';
